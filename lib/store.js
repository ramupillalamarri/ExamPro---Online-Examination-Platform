    ...
    (set, get) => ({
      ...
      fetchData: async (examId = null) => {
        try {
          ...
          const user = get().user;
          const activeRole = get().currentRole || user?.role;
          const userCode = get().currentUserCode || (activeRole === 'student' ? '455770' : null);
          let url = '/api/data';
          if (user) {
            url = `/api/data?userId=${user.id}`;
            if (activeRole === 'student' && userCode) {
      ...
      setCurrentRole: async (role) => {
        const user = get().user;
        const currentCode = get().currentUserCode;
        const targetCode = role === 'student' ? (currentCode || '455770') : (user?.userCode || null);
