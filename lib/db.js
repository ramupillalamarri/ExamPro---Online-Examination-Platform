...
export async function initializeDatabase() {
  ...
  if (process.env.DATABASE_URL) {
    // Skip database check/creation for cloud deployments
    targetPool = new Pool({
      connectionString: process.env.DATABASE_URL,
      ssl: { rejectUnauthorized: false }
    });
  } else {
    // First, verify/create the database locally
  ...
  try {
    ...
    `).catch((err) => console.warn('Could not alter users table avatar_url type:', err.message));
