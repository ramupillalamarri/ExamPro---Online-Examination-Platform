...
def clean_file(file_path):
    ...
    try:
        ...
        for q in questions:
            ...
            if re.search(r'^Question\s+\d+\s+placeholder$', q_text, re.IGNORECASE) or q_text.lower() in ["placeholder", "question placeholder"]:
                q["question_text"] = ""
                cleaned_count += 1
            ...
            for opt in q.get("options", []):
                opt_str = opt.strip()
                if re.search(r'^(Option\s+)?[A-D]$', opt_str, re.IGNORECASE) or opt_str.lower() in [
        ...
        print(f"  Successfully cleaned: {cleaned_count} question texts and {cleaned_opts_count} option texts.")
